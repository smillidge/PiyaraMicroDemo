 ...
 *     Eurotech
 *******************************************************************************/
package fish.payara.piyara.sensehat.sensors;
...
public class HTS221 {
    ...
    private static final Logger s_logger = Logger.getLogger(HTS221.class.getName());
    ...
    public static synchronized HTS221 getHumiditySensor(int bus, int address, int addressSize, int frequency) {
        if (humiditySensor == null && humidityI2CDevice == null) {
            ...
            try {
                ...
                s_logger.log(Level.SEVERE, "Could not create I2C Device", e);
            }
        }
    ...
    public static void closeDevice() {
        try {
            ...
            s_logger.log(Level.SEVERE, "Error in closing device", e);
        }
    }
    ...
    public static int read(int register) {
        ...
        try {
            ...
            s_logger.log(Level.SEVERE, "Unable to read to I2C device", e);
        } catch (InterruptedException e1) {
            s_logger.log(Level.SEVERE, e1.toString());
        }
    ...
    public static void write(int register, byte[] value) {
        try {
            ...
            s_logger.log(Level.SEVERE, "Unable to write to I2C device", e);
        }
    }
    ...
    private void readCalibration() {
        ...
        s_logger.log(Level.INFO, "t0_degC : " + t0_degC);
        s_logger.log(Level.INFO, "t1_degC : " + t1_degC);
        s_logger.log(Level.INFO, "t0_out : " + t0_out);
        s_logger.log(Level.INFO, "t1_out : " + t1_out);

        s_logger.log(Level.INFO, "h0_rh : " + h0_rh);
        s_logger.log(Level.INFO, "h1_rh : " + h1_rh);
        s_logger.log(Level.INFO, "h0_t0 : " + h0_t0);
        s_logger.log(Level.INFO, "h1_t0 : " + h1_t0);

        s_logger.log(Level.INFO, "mt : " + mt);
        s_logger.log(Level.INFO, "qt : " + qt);
        s_logger.log(Level.INFO, "mh : " + mh);
        s_logger.log(Level.INFO, "qh : " + qh);
    }

    private int readHumidity() {
        int hum = read(HUMIDITY_H_REG) << 8 | read(HUMIDITY_L_REG) & 0x000000FF;
        s_logger.log(Level.INFO, "hum : " + hum);
        return hum;
    }

    private int readTemperature() {
        int temp = read(TEMP_H_REG) << 8 | read(TEMP_L_REG) & 0x000000FF;
        s_logger.log(Level.INFO, "temp : " + temp);
        return temp;
    }
}
